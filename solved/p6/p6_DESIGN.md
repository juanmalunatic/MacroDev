...
1. Make summary files display repository-relative paths instead of absolute local Windows paths.
2. Optionally add a raw merged-data diagnostic file before dropping missing/non-positive PWT variables.
3. Optionally create the education-parameter table in Markdown/LaTeX if it helps the final report.
4. Optionally align `p6_AGENTS.md` wording with the actual `p6_AGENTS.md` / `p6_DESIGN.md` filenames.

## Next recommended surgical cleanup

This is the next preferred cleanup if the user asks for one.

### Cleanup goal

Summary files generated by the unified scripts should display repository-relative paths, not absolute local Windows paths.

Currently, some summaries may display paths like:

```text
C:\AcademicRepos\MacroDevBase\solved\p6\code\output\unified\...
```
...
Patch only the unified scripts:
...
Use it only for human-readable summary output and console reporting.

Do not change calculations, data matching, output filenames, figure generation, table generation, or benchmark logic.
...
1. All benchmark checks still pass.
2. Numerical outputs are unchanged up to tolerance.
3. Summary files now use repository-relative paths.
4. The frozen benchmark script remains untouched.
5. Manual LaTeX remains untouched.
